...
console.log(`Total Step 1 questions: ${rows.length}`);
...
for (const [substep, questions] of Object.entries(bySubstep)) {
  const missingInSubstep = questions.filter(q => !q.exists);
  console.log(`=== ${substep} (${questions.length} total, ${missingInSubstep.length} missing) ===`);
  for (const q of questions) {
    if (!q.exists) {
      console.log(`  [MISSING] [${q.id}] ${q.title} | key: ${q.key}`);
    } else {
      console.log(`  [PRESENT] [${q.id}] ${q.title} | key: ${q.key}`);
    }
  }
}
